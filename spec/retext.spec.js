...
describe('Retext()', function () {
    ...
    it('should return a newly initialized `Retext` object, when invoked without arguments', function () {
        assert(new Retext() instanceof Retext);
    });

    it('should set the `parser` attribute to `parse-english`, when invoked without arguments', function () {
    ...
    it('should set the `parser` attribute to the passed in parser, when given', function () {
        var retext = new Retext(noop);
        assert('parser' in retext);
        assert(retext.parser === noop);
    });

    it('should create a new context/parser/textom when required, thus not requiring from memory', function () {
        var rootNode1 = new Retext().parse();
        var rootNode2 = new Retext().parse();
...
describe('Retext#use', function () {
    ...
    it('should throw, when something other than a function was given', function () {
        var retext = new Retext();
        assert.throws(function () { retext.use(); }, /undefined/);
        assert.throws(function () { retext.use(null); }, /null/);
        assert.throws(function () { retext.use(undefined); }, /undefined/);
        assert.throws(function () { retext.use(true); }, /true/);
        assert.throws(function () { retext.use({}); }, /object Object/);
    });
...
describe('Retext#parse', function () {
    ...
    it('should immediately call the `attach` method on a plugin, when `use` is called', function () {
        var retext = new Retext(),
            isCalled = false;
    ...
    it('should not call the `attach` method, when `parse` is called', function () {
        var retext = new Retext(),
            isCalled = false;
    ...
    it('should call `use`d plugins with an instance of RootNode and Retext, when `parse` is called', function () {
        var retext = new Retext(),
            args = null,
            isCalled = false,
            tree;

        function plugin () { args = arguments; }
    ...
    it('should immediately call (during parsing) `use`d plugins, with an instance of RootNode and Retext', function () {
        var retext = new Retext(),
            args = null;

        function nestedPlugin () { args = arguments; }
    ...
    it('should not call (during parsing) `use`d plugins, when already used', function () {
        var retext = new Retext();
